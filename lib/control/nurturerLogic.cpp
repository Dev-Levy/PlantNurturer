...
void NurturerLogic::control(const PlantConfig &config)
{
    ...
    if (millis() - lastSensorRefresh > 500UL)
    {
        ...
        bool wateredEnough = actuator.state.pumpOn && now - actuator.pumpStart >= wateringDurationMs;
        bool canToggleP = now - actuator.pumpWaitTime >= PUMP_COOLDOWN_IN_SECONDS * 1000UL;
        //---------------------------------------------
        bool isThereEnoughLight = data.lightLux > LUX_LIMIT;
        bool shouldHaveLight = isSunnyHour(config);
        bool canToggleL = now - actuator.lightWaitTime >= LIGHT_COOLDOWN_IN_SECONDS * 1000UL;
        //---------------------------------------------
        ...
        bool canToggleF = now - actuator.fanWaitTime >= FAN_COOLDOWN_IN_SECONDS * 1000UL;
        //---------------------------------------------
        bool isSoilTooCold = data.soilTemp < (config.idealSoilTemp - (uint16_t)(CELSIUS_DIFF_ALLOWED * 10));
        bool isSoilTooHot = data.soilTemp > (config.idealSoilTemp + (uint16_t)(CELSIUS_DIFF_ALLOWED * 10));
        bool canToggleH = now - actuator.padWaitTime >= HEATING_COOLDOWN_IN_SECONDS * 1000UL;

        sensor.readAll();
        lastSensorRefresh = millis();
        DEBUG_PRINT(clock.getUnixNow());
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(data.lightLux);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(data.airTemp / 10.0);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(data.airHumidity / 10.0);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(data.soilTemp / 10.0);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(data.soilMoistureAnalog);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(actuator.state.pumpOn);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(actuator.state.lightOn);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(actuator.state.fanOn);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(actuator.state.padOn);
        //-------------------------------------------------------------
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(shouldWater);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(wateredEnough);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(canToggleP); // pump
        //-------------------------------------------------------------
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isThereEnoughLight);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(shouldHaveLight);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(canToggleL); // light
        //-------------------------------------------------------------
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isTooHot);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isTooCold);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isTooHumid);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isHumidityIdeal);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(canToggleF); // fan
        //-------------------------------------------------------------
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isSoilTooHot);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(isSoilTooCold);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINTLN(canToggleH); // pad
    }
...
{
    unsigned long now = millis();
    const uint16_t wateringMillis = plant.getWateringMs(config.waterMl);
    const unsigned long wateringDurationMs = (unsigned long)wateringMillis;
